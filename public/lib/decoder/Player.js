    ...
    if (this._config.useWorker){
      ...
      if (this._config.transferMemory){
        this.decode = function(parData, parInfo){
          ...
          worker.postMessage({buf: parData, offset: 0, length: parData.byteLength, info: parInfo}, [parData]); // Send data to our worker.
        };
        ...
        this.decode = function(parData, parInfo){
          ...
          worker.postMessage({buf: copyU8.buffer, offset: 0, length: copyU8.byteLength, info: parInfo}, [copyU8.buffer]); // Send data to our worker.
        };
